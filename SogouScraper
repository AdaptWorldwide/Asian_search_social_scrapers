...
def Request(keyword):
    requestURL = '{}{}'.format('https://www.sogou.com/web?query=',keyword.replace(' ','+'))
    r = requests.get(requestURL,headers=RandomUserAgent())
    soup = BeautifulSoup(r.text,'lxml')
    return soup, keyword

def GetLinks(soup,keyword):
    ListOfResults = soup.findAll('div',attrs={'class':'fb'})
    FinalResults = []
    for Result in ListOfResults:
        ...
        try:
            ...
            continue
        realLink = link.split('url=')
        print(realLink)
        try:
            linkTrue = (realLink[1])
            linkTrue = parse.unquote(linkTrue)
            linkTrue = linkTrue.split('&did=')
            linkTrue = linkTrue[0]
        except:
            try:
                realLink = link.split('&a=')
                linkTrue = realLink[1]
            except:
                linkTrue = link
        FinalResults.append(linkTrue)
